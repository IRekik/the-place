...
const BlogPosts = () => {
    ...
    return (
        ...
        <div>
            ...
            Threads that everybody loves, threads that you love. Provides the juice in real-time, every time. Grab your pop-corn and hop on.
            </p>
        </div>
        <div className="mt-12 grid gap-4 divide-y md:grid-cols-2 md:divide-y-0 lg:grid-cols-1">
            {visiblePosts.map((item, idx) => (
            <article className="mt-5 pt-8 md:pt-0 border rounded-lg border-gray-200 transition duration-300 transform hover:scale-105" key={idx}>
                <Link legacyBehavior href={`/posts/${item.blog_id}`} passHref>
                <a className="block">
                <div className="ml-2">
                    {item.img_reference && (
                        <img src={item.img_reference} loading="lazy" alt="Img" className="w-20 rounded-lg" />
                    )}
                    ...
                    <div className="mt-2">
                        ...
                        {item.edit_date && ` · Edited ${formatDate(item.edit_date)}`}
                        </span>
