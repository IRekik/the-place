...
describe('uploadBase64Image', () => {
    ...
    it('should upload a base64 image and return the URL', async () => {
      (cloudinary.uploader.upload as jest.Mock).mockResolvedValue(mockApiResponse);
    ...
    it('should handle upload failure and throw an error', async () => {
      ...
      await expect(uploadBase64Image(base64Image)).rejects.toThrowError('Upload failed');
    });
  });