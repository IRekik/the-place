...
describe('Endpoint testing', () => {
    ...
    it('DELETE - /delete-post - should return a 204 status to confirm delete operation', async () => {
        const createPostResponse = await request.post('/submit-data').send({
            title: 'Test Post',
        ...
        const deleteResponse = await request.delete(`/delete-post/${postId}`);    
        expect(deleteResponse.status).toEqual(204);
    });

    // Test case: edit post endpoint, creates a post, then edits it and compares request status, 200 expected
    it('DELETE - /edit-post - should return a 200 status to confirm post editing operation', async () => {
        const createPostResponse = await request.post('/submit-data').send({
            title: 'Test Post',
            text_content: 'Test Content',
            img_reference: ''
        ...
        const postId = createPostResponse.body.blog_id;    
        const editResponse = await request.post(`/edit-post/${postId}`).send({
            title: 'New Post',
    ...
    it('GET - /getAllPosts - should return a list of the blog data', async () => {
        const response = await request.get('/get-hero-data');
    ...
    it('GET - /get-post-by-id - should return a list containing the information of the designated post', async () => {
        ...
        const response = await request.get(`/get-post-by-id/${postId}`);
        expect(response.status).toEqual(200);
